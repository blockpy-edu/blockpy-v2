...
describe("RunCoordinator", () => {
    ...
    it("runs the grader, suppresses its output, and uploads the score", async () => {
        ...
        expect(
            transport.calls.some(
                (call) =>
                    call.route === "blockpy/log_event" && call.params.event_type === "Intervention",
            ),
        ).toBe(true);
    });
    ...
    it("replays the run with collected inputs after input()", async () => {
        const { coordinator, engine, runStore } = makeCoordinator(
            [
                {
                    result: {
                        outcome: { status: "input-pending", prompt: "Name?" },
                        feedback: null,
                    },
    ...
    it("shows runtime errors in the console and feedback", async () => {
        ...
        expect(
            state.entries.some(
                (entry) => entry.kind === "stderr" && entry.text.includes("NameError"),
            ),
        ).toBe(true);
    });
    ...
    it("skips the grade upload when there is no submission id", async () => {
        ...
        await coordinator.run();
        expect(transport.calls.some((call) => call.route === "blockpy/update_submission")).toBe(
            false,
        );
    });
