...
describe('buildVfsFiles', () => {
  ...
  it('projects assignment columns into the instructor namespace', () => {
    ...
    expect(readFile(files, 'instructions.md', 'instructor')?.content).toBe(assignment.instructions);
...
describe('canWrite', () => {
  ...
  const starting: VfsFile = { name: 'b.py', namespace: 'starting', content: '', origin: 'column' };
