...
describe("vfsStore", () => {
    ...
    it("resets student files from the starting namespace", () => {
        ...
        expect(
            files.find((f) => f.namespace === "student" && f.name === "answer.py")?.content,
        ).toBe("start\n");
        expect(
            files.find((f) => f.namespace === "student" && f.name === "notes.txt")?.content,
        ).toBe("template");
    ...
    it("replaces all files and clears dirty state on initialize", () => {
        ...
        store
            .getState()
            .initialize([
                { name: "answer.py", namespace: "student", content: "new", origin: "column" },
            ]);
