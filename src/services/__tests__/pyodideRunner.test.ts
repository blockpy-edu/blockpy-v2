...
import { runPython, isPyodideLoaded, resetPyodide, loadPyodide } from "../python/pyodideRunner";
    ...
    describe("loadPyodide (mocked)", () => {
        it("loads Pyodide and sets instance", async () => {
            ...
            vi.spyOn(document, "createElement").mockReturnValue(
                mockScript as unknown as HTMLElement,
            );
    ...
    describe("runPython with mock pyodide", () => {
        it("runs code and captures stdout", async () => {
            const mockPyodide = {
                ...
                setStdout: vi
                    .fn()
                    .mockImplementation(({ batched }: { batched: (s: string) => void }) => {
                        batched("hello world");
                    }),
                setStderr: vi.fn(),
            };
